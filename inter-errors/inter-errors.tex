...
do not fluctuate over the source tree much. We will see that a low file
...
inappropriate moves of code parts among files in every release is required and
so on.  In other words, every change should have its reason, otherwise the
change will not be accepted by the project management.
...
inspecting these versions, we found out that many of the errors are of the
similar kind like the example above, i.e. cannot be fixed or were already
reported but not fixed yet for some reason.
...
This would not matter unless it is on the same line. In this case it is not,
so in the end both of the cases the entry will look the same in the report
list:
\begin{displaymath}
...
appear in $RL^{filtered}_2$ anymore.

\begin{table*}[tb]
\begin{center}
\begin{tabular}{|l|r||r|r||r||r|r|} \hline
          & \textbf{none} & \textbf{simple} & \textbf{FR} (\%) &
	  \textbf{brutal} & \textbf{advanced} & \textbf{FR} (\%) \\ \hline
...
well. So similarly to $RL_1^{new}$, we define $RL_2^{new} = \{new_2(x)|x \in
RL_1\}$ where $new_2(x) = (s_f(x),s_l(x),s_c(x),s_e(x))$. In this case, $s_f$,
$r_c=s_c$, $r_e=s_e$ are defined as pick-up functions like above. $r_f$
...
defined as:
\begin{displaymath}
RL_2^{filtered} = \{x|x \in RL_2^{new} \wedge x \notin RL_1^{new}\} \\
\end{displaymath}
...
We also expressed the reduction in percents (\emph{Filter Ratio}).  It
designates how many errors were pruned and is computed the standard way as
$100-\frac{100\cdot |RL_2^{filtered}|}{|RL_2|}$. There are two columns
containing them, the first when -rc4+ with \emph{Simple Solution} is
considered, the latter for \emph{Advanced Solution} being used.
...
because this threat is mitigated with our tool thanks to how the preprocessed
output is taken. It is not taken as a whole, but includes, which might change
the line numbers more often are not a part of the checking process, so they
are not involved in the reported lines.

On the other hand, it does not mean this solution is useless. After we fix up
the parser in \st, and consider whole sources, the situation might
change. The same way where instead of \st, some other tool is used.

The script used previously for converting XML to database was augmented to
fetch and store also the line contents from the file into the database.  The
structure of the database table was altered a bit to contain one more field.
So, instead of storing 2-tuple of line and its contents in one column, they
are stored in two separate ones.

The SQL query used to obtain the filtered results is just enhancement of the
former. \textit{line\_contents} is added there to appropriate places.
