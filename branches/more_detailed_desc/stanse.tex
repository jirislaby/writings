...
\begin{abstract}
  \Stanse is a modular framework, available free under the GPLv2 license,
  ...
  extensibility with new bug-finding techniques with a minimal effort.
...
successful tools, like \textsc{CodeSonar}~\cite{codesonar}\todo{Unmatched citation} or
...
tools to specific tasks). The existing free tools are usually severally limited
in what they can do (e.g.~\textsc{Uno}~\cite{uno},
\textsc{Sparse}~\cite{sparse}, \textsc{Smatch}\todo{Missing citation}). One notable exception is
...
Section~\ref{sec:results} presents some results of \Stanse evaluation on the Linux
...
several stages.\todo{Put descriptive text here.}
 %First, the framework asks user for \emph{configuration} of his session with \Stanse consisting of a list of source files to be verified and checkers to be applied. The source files are then parsed and encoded in \emph{abstract syntax tree (AST)}, \emph{control-flow graph   (CFG)}, and \emph{call-graph} internal structures. And finally, the checkers are executed on the internal structures,
%instantiated,\todo{co se mysli tim ``checkers are instantiated''?}\note{Vytvori se jejich instance. Doslova, vytvori se pomoci klicoveho slova new, aby existoval v pameti.} run,
%and error reports are collected and presented back to the user.

\subsection{Verification pipeline}

\subsubsection{Setting Configuration (I): Selecting and Parsing Source Files}

There are several ways to tell \Stanse what source files to analyze. Besides
the standard possibilities, comprising a single given file, all files from a given
...
language family case\todo{Why is there: "In the C
language family case"}, it also remembers compiler flags for preprocessing
purposes. This functionality was inspired by the
\textsc{Sparse}~\cite{sparse} tool. \Stanse can currently process source files written in C language fulfilling the
ISO/ANSI C99 standard and possibly including almost all of the GNU C extensions.\todo{Why is here the footnote? I hink, it is inappropriate here.}\footnote{In principle, the \Stanse framework can handle any programming language for which a frontend is written. We are currently working on C++ support.}
%Before parsing, the sources are preprocessed by the standard GNU C preprocessor. code is then parsed into three internal structures: AST, CFG, and call-graph. All these structures can be dumped in a textual or graphical form.
...
\subsubsection{Setting Configuration (II): Specifying checkers}

An user can choose, which checkers integrated into \Stanse should be applied to selected source files. This is very simple procedure for the user, since it only consists of choosing appropriate checkers from a list of checkers, plus choose for each checker, whether interprocedural or intraprocedural variant of a checker should be used.

\subsubsection{Executing Configuration}

When an user completes a configuration, it can be executed. It means, that all the checkers in the configuration are executed on each source file in the configuration. The user can watch the progress of the execution in the output window. When the execution is finished, the user receives a complete list of error traces found by checkers.

\subsubsection{Processing Error Traces}
...
interface.
%\note{Tady to zni, jako by soucasti \Stanse byl i ten web interface. Co presne dela ten creator?  Nestaci napsat ``For example, we supply a tool translating the XML file into an \textsc{SQLite} database.''?}\todo{JS: Taky, ze ano, primitivni, ale je :). Viz \url{http://decibel.fi.muni.cz/~xslaby/stanse/}. Creator dela presne ten preklad XML do db, jak pises v te vete.}
...
As mentioned earlier, the design of the framework focus at large software projects.
 Such projects consist of hundreds or
...
in. The implemented least-recently-used algorithm discards the
...
corresponding module is streamed back in. The streaming
is completely transparent to checkers. It means that checkers may assume all the modules are
loaded in the memory all the time. It is framework's responsibility to optimally plan streaming of modules in the memory to provide checkers the illusion that all the modules are in the memory.\todo{Co se mysli tim
...
framework. A checker can simply use this functionality by
specifying
\begin{itemize}
...
\item if the interprocedural walk-through should be performed or
  not, and
\item derive a visitor object whose callback method \texttt{visit()} will be called for each visited node in CFG.\todo{Nepatri
...
\subsection{Integrating New Checker}

\todo{Insert short description how easy one can add new checker into framework.}

\subsection{Statistics of Error Reports}

\todo{Say few words about statistic library - what it can do.}

\subsection{IDE integration}

The last but not least, \Stanse provides an interface for IDE plug-in to be
added as an extra module.
\todo{Say that framework can be taken away from \Stanse and simply included as a library in some plug-in project.}
