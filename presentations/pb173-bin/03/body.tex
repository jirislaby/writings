...
\begin{frame}
  ...
  \begin{textblock*}{4cm}(.65\textwidth,2.8cm)
    \only<2->{
...
\begin{frame}
  ...
  \heading{Demo:} \textsc{DosBox} a:
  \begin{itemize}
    \item Reboot přes COM (\code{ea f0 ff 00 f0})
    \item Výpis (\code{b8 31 0e 30 ff cd 10 eb f7})
  \end{itemize}
...
\begin{frame}{Úkol}
  ...
  \heading{Hexdump sekcí\iffimuni (součást domácího)\fi}
  \begin{enumerate}
