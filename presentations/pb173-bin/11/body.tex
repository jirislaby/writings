...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Vypište, co je na první a druhé pozici pole \code{uint32_t}
...
\begin{frame}[fragile]{Iterace přes PCI zařízení}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Bere \code{struct pci_device_iterator *}
  ...
  \begin{block}{Struktury \code{pci_id_match} a \code{pci_slot_match}}
    \begin{tabular}{lp{.13\textwidth}lp{.08\textwidth}l}
      \begin{lstlisting}
uint32_t vendor_id;
uint32_t device_id;
...
\begin{frame}
  ...
  \item Mapování regionu: \code{int pci_device_map_range(struct pci_device *dev,
        pciaddr_t base, pciaddr_t size, unsigned map_flags, void **addr)}
    \begin{itemize}
      \item Mapuje region do paměti
      \item Podobné \code{mmap} regionu z dnešního cvičení
      \item \code{base} a \code{size}: z \code{pci_device->region[X]}
      \item \code{map_flags}: \code{PCI_DEV_MAP_FLAG_WRITABLE}
    \end{itemize}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Počkejte, než bude devátá pozice \code{0}
