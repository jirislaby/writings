...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item \textsc{AntlrWorks}: \href{http://www.antlr3.org/download/antlrworks-1.5.1.jar}{,,Version 1.5''}
      \item \ant: \href{http://www.antlr3.org/download/antlr-3.5.2-complete.jar}{,,Complete \ant 3.5.2 Java binaries jar''}
    \end{itemize}
  ...
  \item Prohlédněte si gramatiku v \textsc{AntlrWorks}
  \end{enumerate}
...
\begin{frame}[fragile]{Formát \ant}
  ...
  \begin{lstlisting}[language=, columns=fullflexible,belowskip=0pt]
...
options {
  ...
  \end{lstlisting}
  \pause
  \begin{lstlisting}[language=, columns=fullflexible,aboveskip=0pt]
@header {
#define X	10
...
\begin{frame}[fragile]
  ...
  \heading{\ant obsahuje obě části parseru v jednom souboru}
  \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Implicitně jménem tokenu/pravidla \tikz[baseline=-.1ex,remember picture] \node [coordinate] (IMPL) {};

      \pause

    \item \tikz[baseline=-.5ex,remember picture,inner sep=0pt,minimum height=0pt] \node (EXPL) {Nebo pojmenováním};
    \end{itemize}
...
helloWorld : x=AHOJ y=SVETE {
  ...
  \end{lstlisting}
  \onslide<2->
  \begin{lstlisting}[language=, columns=fullflexible,aboveskip=0pt,belowskip=0pt]
      printf("whole=\%s x=\%s y=\%s\n", (*@\tikz[remember picture,baseline=-1.8ex,xshift=.7em] \node [coordinate] (EXPLT1) {};@*)$x.text->chars, (*@\tikz[remember picture,baseline=-1.8ex,xshift=.7em] \node [coordinate] (EXPLT2) {};@*)$y.text->chars,
            (*@\tikz[remember picture,baseline=2pt,xshift=5.1em] \node [coordinate] (IMPLT) {};@*)$helloWorld.text->chars);
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \path<3->[arr] (IMPL) edge[out=340,in=330,looseness=2.8] (IMPLT);
    \path<4->[arr] (EXPL.south east) edge[out=270,in=130,shorten < = 1pt] (EXPLT1);
    \path<4->[arr] (EXPL.south east) edge[out=340,in=110,shorten < = 1pt] (EXPLT2);
  \end{tikzpicture}
...
\begin{frame}[fragile]
  ...
  \heading{Každé pravidlo může akceptovat/vracet hodnotu}
  \begin{itemize}
...
expr[int par] returns [int ret]
  ...
  \end{lstlisting}
  \begin{onlyenv}<1>
  \begin{lstlisting}[showlines,language=, columns=fullflexible,aboveskip=0pt,belowskip=0pt]

  \end{lstlisting}
  \end{onlyenv}
  \begin{onlyenv}<2>
  \begin{lstlisting}[language=, columns=fullflexible,aboveskip=0pt,belowskip=0pt]
          $par * atoi($n1.text->chars) + atoi($n2.text->chars);
  \end{lstlisting}
  \end{onlyenv}
  \begin{onlyenv}<3->
  \begin{lstlisting}[language=, columns=fullflexible,aboveskip=0pt,belowskip=0pt]
...
\begin{frame}[fragile]
  ...
  \heading{Každé pravidlo může mít inicializace/ukončení}
  \begin{itemize}
