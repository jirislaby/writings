...
\begin{frame}{Řešení chyb souběhu}
...
\begin{frame}[fragile]{Atomické operace}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Přičtěte 3
    \item Odečtěte 1
    \item Přečtěte hodnotu a vraťte jako návratovou z \code{module_init}
  \end{enumerate}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Pole \code{a} typu \code{long} o velikosti
        \code{1000/BITS_PER_LONG}
    \end{itemize}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item \code{bitmap_scnprintf} (do 4.0), \code{\%100pb} (od 4.0)
    \end{itemize}
...
\begin{frame}
  ...
  \begin{center}
    \heading{\pozor \\ Zámky lze držet jen v jádře (po dobu vykonávání 1
      syscallu)}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]{Spinlocky}
  ...
  \begin{itemize}
    ...
    \item \code{DEFINE_SPINLOCK(lock)}$_S$, \code{spinlock_t lock}$_D$
...
\begin{frame}
  ...
  \item \code{atomic_t} změňte na obyčejný \code{int}
...
\begin{frame}[fragile]{Mutexy}
  ...
  \begin{itemize}
    ...
    \item \code{DEFINE_MUTEX(lock)}$_S$, \code{struct mutex lock}$_D$
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \code{DEFINE\_SEMAPHORE(lock)}
...
\begin{frame}[fragile]{Deadlock}
  ...
  \begin{figure}
    \includegraphics[width=11em]{deadlock}

    \iffalse
    \tikzstyle{road} = [draw,semithick]
    \tikzstyle{kapota} = [draw]
    \tikzstyle{podvozek} = [fill=black!55!green, rounded corners=2.5pt,
      minimum width=14.5mm,
      minimum height=2mm]
    \tikzstyle{kolo} = [fill=black, circle, minimum size=4.5pt]
    \begin{tikzpicture}[minimum size=0pt, inner sep=0pt, node distance=5mm]
      \node (TL1) {};
      \node (TL2) [right=of TL1] {} edge[road] (TL1);
      \node (TL3) [above=of TL2] {} edge[road] (TL2);

      \node (TL4) [right=of TL3] {};
      \node (TL5) [below=of TL4] {} edge[road] (TL4);
      \node (TR1) [right=2cm of TL5] {} edge[road] (TL5);
      \node (TR2) [above=of TR1] {} edge[road] (TR1);

      \node (TR3) [right=of TR2] {};
      \node (TR4) [below=of TR3] {} edge[road] (TR3);
      \node (TR5) [right=of TR4] {} edge[road] (TR4);

      \node (TR6) [below=of TR5] {};
      \node (TR7) [left=of TR6] {} edge[road] (TR6);
      \node (BR1) [below=2cm of TR7] {} edge[road] (TR7);
      \node (BR2) [right=of BR1] {} edge[road] (BR1);

      \node (BR3) [below=of BR2] {};
      \node (BR4) [left=of BR3] {} edge[road] (BR3);
      \node (BR5) [below=of BR4] {} edge[road] (BR4);

      \node (BR7) [left=of BR5] {};
      \node (BR6) [above=of BR7] {} edge[road] (BR7);
      \node (BL1) [left=2cm of BR6] {} edge[road] (BR6);
      \node (BL2) [below=of BL1] {} edge[road] (BL1);

      \node (BL3) [left=of BL2] {};
      \node (BL4) [above=of BL3] {} edge[road] (BL3);
      \node (BL5) [left=of BL4] {} edge[road] (BL4);

      \node (BL6) [above=of BL5] {};
      \node (BL7) [right=of BL6] {} edge[road] (BL6);
      \node (TL6) [above=2cm of BL7] {} edge[road] (BL7);
      \node (TL7) [left=of TL6] {} edge[road] (TL6);

      \node (MID1) [right=of TL6] {};
      \node (MID2) [right=2cm of MID1] {} edge[road] (MID1);
      \node (MID3) [below=2cm of MID2] {} edge[road] (MID2);
      \node (MID4) [left=2cm of MID3] {} edge[road] (MID3) edge[road] (MID1);

      \path (TL1) -- (TL7) node[midway] (CEN) {};

      \foreach \xoff/\yoff/\rot in {0mm/1.7mm/0, 16mm/1.7mm/0,
	  10mm/26.7mm/0, 26mm/26.7mm/90} {
	  \node[podvozek, below right=\yoff and \xoff of TL1] (PODVOZEK) {};

	  \node[kolo, left=8pt of PODVOZEK.south] {};
	  \node[kolo, right=8pt of PODVOZEK.south] {};

	  \node[right=1mm of PODVOZEK.north west] (KAP1) {};
	  \node[above right=1mm and 4mm of KAP1] (KAP2) {} edge[kapota] (KAP1);
	  \node[right=4mm of KAP2] (KAP3) {} edge[kapota] (KAP2);
	  \node[left=3mm of PODVOZEK.north east] (KAP4) {} edge[kapota] (KAP3);
      }
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item Jedno implementuje \code{.read}
      \item Jedno implementuje \code{.write}
    \end{itemize}
