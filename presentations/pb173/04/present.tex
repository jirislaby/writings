...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Nastavení 2., 63.\ a 76.\ bitu (čísla považujte za indexy v C)
    \item Výpis \emph{longu} (\code{\%lx}) s 63.\ bitem
    \begin{itemize}
      \item \code{bitmapa[BIT_WORD(63)]}
    \end{itemize}

    \item Výpis celé bitmapy jako řetězce
    \begin{itemize}
      \item \code{bitmap_scnprintf} (do 4.0), \code{\%*bp} (od 4.0)
    \end{itemize}

    \item Výpis \emph{longů} obsahující bity nastavené na 1
    \begin{itemize}
      \item \code{for_each_set_bit}
    \end{itemize}

    \item Výpis \emph{pozice} 1.\ nastaveného bitu
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Historický
    \begin{itemize}
      \item Dnes už v jádře nenajdeme (commit 4ba8216cd905 v 2.6.39)
...
\begin{frame}{Úkol}{Součást domácího}
  ...
  \item Spí 20~ms po každém zápisu \emph{znaku} do bufferu (\code{get_user} a
    \code{msleep} z \header{linux/delay.h})
  \end{itemize}
  \item Čtení
  \begin{itemize}
