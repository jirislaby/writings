...
\begin{frame}
  ...
  \item Na x86: řadič klávesnice, PC spkr, staré časovače a ovladače přerušení,
    ladicí port (\code{0x80} $\Rightarrow$ segmentový displej na desce), \dots
  \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \cmd{lspci -nn} a najít EDU a jeho vendor+device ID
  \end{itemize}
  ...
  \end{itemize}
  \item Zavolat \code{pci_register_driver} a \code{pci_unregister_driver}
  \end{enumerate}
...
\begin{frame}
  ...
  \begin{itemize}
  \item USB, I$^2$C, HID, IEEE1394, ACPI, INPUT, EISA, \dots
...
0x0020 & 4B  & R   & Status    \\ \hline
\end{tabular}
\caption{Specifikace baru 0 karty EDU}
\end{table}

\begin{itemize}
\item ID \& Revision: \code{0xRRrr00edu}, RR -- major, rr -- minor
\item Inverted value: zapsané číslo se obrátí (operátor \code{\~})
\item Faktoriál: vypočte se faktoriál (je nutné počkat na status bit)
\item Status: 0. bit -- probíhá výpočet faktoriálu
\end{itemize}
...
\begin{frame}
  ...
  \item Přečíst a rozkódovat identifikaci a revizi (\code{readX})
  \item Ověřit živost karty pomocí invertovacího registru
