...
int my_mmap(struct file *filp, struct vm_area_struct *vma)
{
  if ((vma->vm_flags & (VM_WRITE | VM_READ)) != VM_READ)
    return -EINVAL;
  return remap_pfn_range(vma, vma->vm_start, mem, PAGE_SIZE, vma->vm_page_prot);
...
\begin{frame}
  ...
  \begin{enumerate}
  \item Alokovat 2 a 2 stránky (vmalloc a page alokátory)
  \item Vyplnit libovolnými řetězci, každou stránku různými
  ...
  \begin{itemize}
  \item Vypsat řetězce na pozicích 0 a 4096 z obou mapování
  \end{itemize}
  \end{enumerate}
...
\begin{frame}
  ...
  \end{itemize}
  \item V \texttt{mmap} je třeba zkontrolovat rozsahy a velikosti
  \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  \pause
  \item Zapnutí ,,správování sběrnice''
  \begin{itemize}
...
\begin{frame}
  ...
  \item Počkat na nulovou hodnotu bitu RUN (na dokončení přenosu)
  \end{itemize}
...
\begin{frame}
  ...
  \heading{DMA s přerušením} (součást domácího)
  \begin{enumerate}
