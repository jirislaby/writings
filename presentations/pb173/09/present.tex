...
\section{Mapování paměti jádra (\code{mmap})}
\frame{\sectionpage}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \emph{Syscall:} \code{mmap}
  \end{itemize}

  \begin{block}{Prototyp systémového volání \code{mmap}}
  \begin{lstlisting}
void *mmap(void *addr,
    size_t len,
    int prot,
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item 2$\times$ \code{vmalloc(PAGE_SIZE)} a 2$\times$ \code{__get_free_page}
    \end{itemize}
