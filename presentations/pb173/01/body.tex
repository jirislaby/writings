...
\begin{frame}{S čím budeme pracovat?}
  ...
  \heading{Hardware}
  \begin{itemize}
    \item Stroje \iffimuni satyr01--10 \fi
    \begin{itemize}
      \item CentOS \iffimuni 6.x \else 7.x \fi
      \item Login/heslo: \iffimuni vyvoj/vyvoj \else student/student \fi
      \iffimuni {
    ...
    \begin{itemize}
      \item \textsc{Qemu} -- HW k práci s PCI, I/O, na přerušení, DMA\dots

      \pause

      \item Obraz openSUSE Leap
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Vygenerujte záplaty ze 2 posledních změn (\cmd{git format-patch -2})
    \item Prostudujte výstupní soubory
...
\begin{frame}
  ...
  \item Proveďte ,,\cmd{readelf -p}'' sekce ,,\file{.modinfo}''
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Kód
    \begin{itemize}
      \item Hledání: \url{https://elixir.bootlin.com}
    \end{itemize}
  \end{itemize}
  ...
  optimalizovanou pro \xesl (v \file{arch/x86/lib/}) implementaci
  \code{strlen}
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item \file{qemu-start} z repozitáře
      \item Parametrem je cesta k obrazu
    \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item \pozor: omezený zásobník (4--16\,K)
    \begin{itemize}
      \item Žádná nebo malá rekurze
      \item Jen pro malá data (\code{int}, malé \code{struct}, krátká pole,
	\dots)
    \end{itemize}
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    ...
    \item Vypište paměť jako řetězec (\code{pr_info} a \code{\%s})
    \item Uvolněte paměť
