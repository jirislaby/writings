...
\begin{tikzpicture}
  ...
  \begin{pgfonlayer}{ram}
    \node [draw,fill=blue!15,spcwid,minimum height=5.3cm,below=0mm of PHYS.north] (RAM) {RAM};
  \end{pgfonlayer}
  ...
  \node [box,spcwid,minimum height=4.3cm,align=center,below=0mm of VIRT.north] (US)
    {Userspace \\ (differs per process)};
  \begin{pgfonlayer}{ram}
    \node [box,fill=blue!15,spcwid,minimum height=1.2cm,below right=0mm of US.south west,baseline=-5ex] (PHYS2) {};
    \node [above=0mm of PHYS2.south] {Physical memory};
  \end{pgfonlayer}
  \node [box,below right=0mm of US.south west] (HOLE) {Hole};
  \node [box,spcwid,above=0mm of VIRT.south] (MODS) {Modules};
  \node [box,spcwid,above=0mm of MODS.north] (KERN) {Kernel};
  \node [box,above left=0pt of KERN.north east] {Hole};
  ...
  \draw (PHYS.north east) -- (HOLE.south west);
