...
\begin{frame}[fragile]
  \frametitle{Hledání zařízení}
  \iffimuni\framesubtitle{Událostmi}\fi
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item Číslo sběrnice: \code{pdev->bus->number}
      \item Číslo zařízení: \code{PCI_SLOT(pdev->devfn)}
      \item Čislo funkce: \code{PCI_FUNC(pdev->devfn)}
    \end{itemize}
...
\begin{frame}{Práce s PCI}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Do té doby nelze některé vlastnosti \code{pdev} používat
        (irq)
    \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Např.\ USB, I$^2$C, HID, IEEE1394, ACPI, INPUT, PLATFORM, \dots
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Přemapujte bar 0 (\code{pci_ioremap_bar})
