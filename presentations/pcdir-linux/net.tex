...
\begin{frame}{Programování v síti}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \emph{IPv4, IPv6}
      \begin{itemize}
	\item DHCP, ICMP, \emph{TCP}, \ifpcdirlinuxb \emph{UDP} \else UDP \fi
      \end{itemize}
...
\begin{frame}{Operace nad sockety}
  ...
  \begin{itemize}
    ...
    \item Multiplexing
    \begin{itemize}
      \item Standardně (\code{select}, \code{poll}, \code{epoll})
    \end{itemize}
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item S větším bufferem, např.\ 128 B
    \end{itemize}
    ...
    \begin{itemize}
      \item Vysvětlete
      \item Bude se lišit \code{SOCK_DGRAM}?
    \end{itemize}
  \end{enumerate}
...
\begin{frame}[fragile]{Získání adresy}
  ...
  \begin{block}{Prototypy}
    ...
    \begin{lstlisting}[aboveskip=3pt,belowskip=0pt]
struct addrinfo {
    \end{lstlisting}
    \begin{tabular}{ll}
...
const char *gai_strerror(int errcode);
  ...
  \begin{itemize}
    ...
    \item Návratová hodnota: 0 je OK, jinak číslo chyby (\code{gai_strerror})
  \end{itemize}
...
\begin{frame}[fragile]{Server}
  ...
  \begin{block}{Prototypy}
  \begin{lstlisting}[columns=fullflexible,aboveskip=2pt,belowskip=0pt]
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen);
  \end{lstlisting}
  \onslide<2->
  \begin{lstlisting}[columns=fullflexible,aboveskip=0pt,belowskip=0pt]
int listen(int sockfd, int backlog);
  \end{lstlisting}
  \onslide<4->
  \begin{lstlisting}[columns=fullflexible,aboveskip=0pt,belowskip=2pt]
...
\begin{frame}{Multiplexing}
  ...
  \begin{itemize}
    ...
    \item Funkce \code{select}, \code{poll} a \code{epoll}
  \end{itemize}
...
\begin{frame}{Výhody a nevýhody \code{select}}

  \heading{Nevýhody}
  \begin{itemize}
    \item Jádro modifikuje bitové masky
    \begin{itemize}
      \item Nutno nastavit pokaždé znovu
    \end{itemize}

    \pause

    \item Nutnost projít všechny bitové masky
    \begin{itemize}
      \item A na každý bit se dotázat \code{FD_ISSET}
    \end{itemize}

    \pause

    \item Omezení na počet deskriptorů
    \begin{itemize}
      \item Na linuxu 1024 (\code{FD_SETSIZE})
    \end{itemize}

    \pause

    \item Nelze zavřít ani přidat deskriptor, na který se čeká
    \begin{itemize}
      \item Např.\ z jiného vlákna
    \end{itemize}

    \pause

    \item Nutno spočítat nejvyšší deskriptor
  \end{itemize}
...
if (poll(pfd, 3, -1) > 0)
  ...
  \item \code{revents} ve \code{fds} jsou po návratu patřičně upraveny
  \end{itemize}

\end{frame}

\begin{frame}{Výhody a nevýhody \code{poll}}

  \heading{Narozdíl od \code{select}}
  \begin{itemize}
    \item Nemodifikuje vstupní data
    \begin{itemize}
      \item Deskriptory a \code{events} se nemění
      \item Ale je třeba pokaždé nastavit \code{revents} na 0
    \end{itemize}

    \pause

    \item Stále nutnost projít všechny struktury \code{fds}
    \begin{itemize}
      \item A ne jednodušší bitová pole
      \item Jádro také teď musí procházet struktury
    \end{itemize}

    \pause

    \item Stále nelze zavřít ani přidat deskriptor, na který se čeká

    \pause

    \item Timeout je v ms
  \end{itemize}
...
\begin{frame}[fragile]{\code{epoll}}
...
struct epoll_event {
  ...
  epoll_data_t data;   // user data (union: ptr, fd, u32, u64)
};
    \end{lstlisting}
    \onslide<1->
    \begin{lstlisting}[aboveskip=0pt,belowskip=0pt]
...
int epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout);
  ...
  \begin{enumerate}
    \item Vytvoření: \code{epoll_create1}

    \pause

    \item Úprava seznamu fd: \code{epoll_ctl}
    \begin{itemize}
      \item \code{op}: \code{EPOLL_CTL_ADD}, \code{EPOLL_CTL_MOD},
	\code{EPOLL_CTL_DEL}
      \item \code{fd}: deskriptor, který se má sledovat
      \item \code{event}: co se má sledovat + data
    \end{itemize}

    \pause

    \item Čekání: \code{epoll_wait}
    \begin{itemize}
      \item \code{events}: pole, které se plní daty
      \item \code{maxevents}: velikost pole
      \item Návratová hodnota: počet naplněných prvků do pole
    \end{itemize}
  \end{enumerate}

\end{frame}

\begin{frame}[fragile]{\code{epoll} příklad}
...
int ep = epoll_create1(0);
  ...
  \begin{lstlisting}
for (int a = 0; a < 3; a++)
  epoll_ctl(ep, EPOLL_CTL_ADD, epe[a].data.fd, &epe[a]);
  \end{lstlisting}

  \pause

  \begin{lstlisting}
if (epoll_wait(ep, epe, 3, -1) > 0)
  puts("some event!");
  \end{lstlisting}
...
close(ep);
  ...
  \begin{center}
    \heading{\code{epoll} existuje jen na Linuxu!}
  \end{center}
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      \item Použijte \code{select}, \ifpcdirlinuxb \else nebo \fi
	\code{poll}\ifpcdirlinuxb, nebo \code{epoll} \fi
    \end{itemize}
