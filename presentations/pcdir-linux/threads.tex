...
\begin{frame}{Vytvoření vlákna}
  ...
  \begin{itemize}
    ...
    \item Vytvoření vlákna: \\ {\footnotesize
    \code{int pthread_create(pthread_t *thread, const
      pthread_attr_t *attr,}\\
    ...
    \item Čekaní na skončení: \\ {\footnotesize
      \code{int pthread_join(pthread_t thread, void **retval)}
    }
    \begin{itemize}
      \item \code{thread}: držátko z \code{pthread_create}
      \item \code{retval}: co vrátila \code{start_routine} pomocí \code{return}
      \item Návratová hodnota: \code{0} -- v pořádku, jinak číslo chyby
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Úkol}
  ...
  \begin{enumerate}
    \item Otevřete si \ifpcdirlinux \file{14/main.pro} \else
      \file{04/threads.c}  \fi
