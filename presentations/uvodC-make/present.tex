...
\begin{frame}[fragile]{Makefile}{Pravidla}
  ...
  \begin{onslide}<1->
    \begin{block}{Příklad souboru \file{Makefile}}
...
\begin{frame}[fragile]{Makefile}{Proměnné}
  ...
  \begin{itemize}
    \item Ovlivnění překladu
    \begin{itemize}
      \item Jiný překladač, linker, vlaječky, \dots
    \end{itemize}
    ...
    \begin{itemize}
      \item Na příkazové řádce: \cmd{make all CC=gcc}
      \item V souboru \file{Makefile}: \code{CC=gcc}
    \end{itemize}
...
binary: main.o impl.o
       $(CC) $(CFLAGS) -obinary main.o impl.o
...
\begin{frame}[fragile]{Překladový systém \cmake}
  ...
  \begin{block}{Příklad souboru \file{CMakeLists.txt}}
  \begin{lstlisting}[language=make,columns=fullflexible]
