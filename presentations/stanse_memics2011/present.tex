...
\author[Obdrzalek, Slaby, Trtík]{Jan~Obdrzalek, \emph{Jiri~Slaby},
Marek~Trtík\\ \hskip 1.5em \small{\textit{[Yiri Sla-be]}}}

\institute[ITI FI MU]{ITI, Faculty of Informatics \\ Masaryk University
\\ Czech Republic}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Watches locks, memory accesses, reference counts, \ldots
  \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{center}
  \begin{tabular}{|c|c|} \hline
  20$\times$ in code & 1$\times$ in code \\ \hline\hline
  \begin{lstlisting}[language=C]
lock(A);
glob_var = 1;
unlock(A);
...
\begin{frame}
  ...
  Over 120 errors of the following kind (reported and fixed):
  \begin{itemize}
  \item Memory errors
  ...
  \heading{Other code}
  \begin{itemize}
  \item CESNET project developing HW akcelerators
