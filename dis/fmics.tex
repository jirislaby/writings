...
static program analysis tools like \xgcc~\cite{metal},
...
execution, which we extensively described in Section~\ref{sec:se}. But the
symbolic execution is known to suffer from slowness on large programs or
...
it is a symbiosis of techniques and it also points to the symbolic execution.
This work is based on our paper published in the Springer's Lecture Notes in
Computer Science~\cite{symbiotic} and presented at the 17th International
Workshop on Formal Methods for Industrial Critical Systems 2012. Later, we
also presented the work at the Annual Doctoral Workshop on Mathematical and
Engineering Methods in Computer Science 2012.
...
in \xgcc, \textsc{Esp}, and \textsc{Stanse} and show an example where it does
not work. The example will accompany us through the following sections to
demonstrate each part of the technique. So let us consider the automaton
$\SA(x)$ of Figure~\ref{fig:sm}.  \begin{figure}[htb] \begin{center}
\tikzstyle{loc} = [circle,thick,draw,minimum size=8mm] \tikzstyle{err} = [loc,
fill=red!10, draw=red] \tikzstyle{pre} = [<-, shorten <=1pt, >=stealth',
semithick] \tikzstyle{post} = [pre, ->] \begin{tikzpicture}[node distance=8mm]
\node (S) {}; \node [loc] (U) [right=6mm of S] {\code{U}} edge [pre] node {}
(S); \node [loc] (L) [right=18mm of U] {\code{L}} edge [pre,bend left=20] node
[label=below:\code{lock($x$)}] {} (U) edge [post,bend right=20] node
[label=above:\code{unlock($x$)}] {} (U); \node [err] (DU) [below=of U]
{\code{DU}} edge [pre] node [label=left:\code{unlock($x$)}] {} (U); \node
[err] (DL) [below=of L] {\code{DL}} edge [pre,near start] node
[label=left:\code{lock($x$)}] {} (L); \node [err] (RL) [right=6mm of DL]
{\code{RL}} edge [pre,bend right=10,near start] node [label=above
right:\code{return}] {} (L); \end{tikzpicture} \end{center} \caption{The state
automaton $\SA(x)$ describing errors in manipulation with the lock $x$. The
nodes \code{U} and \code{L} refer to states \emph{unlocked} and \emph{locked},
respectively. The other three nodes refer to error states: \code{DU} to
\emph{double unlock}, \code{DL} to \emph{double lock}, and \code{RL} to
\emph{return in locked state}. The initial node is \code{U}.} \label{fig:sm}
\end{figure} It describes a lock manipulation including malign transitions.
Intuitively, the automaton represents possible courses of states of a lock
referenced by $x$ along an execution of a program.  The state of the lock is
changed according to the program behaviour.  Whenever the program contains a
statement syntactically subsuming the label of a transition, the transition is
fired in the state automaton. We would like to decide whether there exists any
program execution where an instance of the automaton $\SA(x)$ reaches an error
state for some lock in the program.  Unfortunately, this is not feasible due
to potentially unbounded number of executions and unbounded execution length.
...
To sum up, static analysis tools like \xgcc, \textsc{Esp}, and \stan are
highly flexible, fast and thus applicable to extremely large software projects
like the Linux kernel. The tools examine whole code and find many error
reports. Unfortunately, many of the reports are false positives. So the tools
have to use many techniques for partial elimination of false positives.  The
main source of false positives is related to the fact that the analysis does
not work with data values. In particular, the analysis does not track
connections between variable values and states of automata.  Therefore, when a
branching statement is processed, the analysis cannot decide which states of
automata are reachable on the positive branch and which on the negative one.
It may be illustrated by the \emph{double unlock} false positive detected in
the function of Figure~\ref{fig:copy} because the analysis does not know that
the condition on line~\ref{line:copy_if2} holds only if the automaton
$\SA(\var{L})$ is in state $L$. As manual sorting of error reports containing
a pile of false positives is tedious work, the practical applicability of such
tools is limited.
...
produced by other tools (e.g.\ \xgcc or \stan). For each such an error report,
we may instrument the corresponding code only with the state machine
...
calls to \code{lock} and \code{unlock} functions.
...
original program does not halt.  As a result, an input causing an infinite run
of the original program can induce a finite run in the sliced program.
Moreover, the finite run can contain locations which would not be visited by
the infinite run.  This is the only principal source of potential false
positives in our technique.
...
\code{Kleerer}. The modules are in a single library which can be loaded by the
\llvm's \code{opt} program. It loads, the library and processes all input
files as is specified on the command line. If we wanted to run the three
modules on \llvm bytecode stored in the file \code{code.o}, the command would
look like (slicer is selected to be inter-procedural):
\begin{center}
\begin{tabular}{c} {
\begin{lstlisting}[language=bash, numbers=none, frame=none,
	columns=fullflexible]
opt -load LLVMSlicer.so -prepare -slice-inter -kleerer code.o
\end{lstlisting} }
\end{tabular}
\end{center}

The \llvm framework provides us with the C compiler \clang~\cite{clang}. The
\llvm bytecode is generated using the \code{-emit-llvm} option. All other
options to \clang and \code{opt} can be found in the \llvm
documentation~\cite{llvm}. After the code is run through the three modules as
shown above, we use an existing symbolic executor for \llvm called
\klee~\cite{klee}. The whole pipeline is depicted in
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[node distance=6mm, font=\sffamily]
      ...
      \draw [tl_edge] ([yshift=-2mm]KLEERED.west) --
	([yshift=-2mm] KLEERER.east);
      \draw [tl_edge] ([yshift=2mm]KLEERED.west) --
	([yshift=2mm] KLEERER.east);
...
\item It finds assembler patterns and replaces them by pure \llvm code when
they are recognized.

\item Finally, it establishes the set of starting functions similarly to \stan
(Chapter~\ref{sec:stanse}). Starting functions are the roots in the call
graph.

\end{itemize}

\subsection{Slicer}\label{sec:fmics_slicer}
...
structure first. For this purpose, we have a submodule in our slicing library
which we call \code{CreateHammockCFG}. An additional option to the \code{opt}
command has to be added before \code[language=bash]{-slice-inter} to run this
additional pass: \code[language=bash]{-create-hammock-cfg}.
...
function \code{main} that calls a particular starting function.  The
\code{main} function also allocates a symbolic memory for each parameter of
the starting function. Size of the allocated memory is determined by the
parameter type.  More problematic is a pointer as an argument of the starting
function. We have no way to find out whether the pointer points to a single
value in memory or to a whole array. To overcome the problem, we assume there
are at most 4096 members in the array. So for example, four bytes are
allocated for an integer and $4\cdot4096=16384$ symbolic bytes for a pointer
to an integer. The idea behind the array allocations is taken over from the
paper introducing \ucklee~\cite{ucklee}.
...
Since the tool is intended for use on large projects, this section introduces
the reader to the use of \symbio and description of its parts. First, we
developed several scripts in the tool.  They are written in \textsc{Bash} and
\perl and we describe them below, one by one.  There are not only supporting
scripts. We have another \llvm module in \symbio apart from the three modules
described in previous sections. The module is intended to count statistics and
report other useful information. It is called \code{ModStats} and has three
main tasks.  First, it prints how many functions and instructions are in a
module.  Second, it counts and reports how many of the functions contain
assembler, loops, locking functions and calls to external (undefined)
functions. We use the information later in the evaluation phase in
Section~\ref{sec:fmics_experiments}. Finally, the module can dump a transitive
call relation, i.e.\ what is called by what function.

\begin{description}
\item[\textsf{get\_starting\_funs}] This script returns for each file and
function on the input a file name that is generated by the \code{Kleerer}
module.  In other words it picks up one of the starting functions generated by
\code{Kleerer}. 

\item[\textsf{i-builder}] It uses the \textsc{make} build system and generates
preprocessed (\code{.i}) files from all sources in C in the current directory
and subdirectories. The preprocessed files can be later used by
\code{llvm-builder} below.

\item[\textsf{jf-parser}] This scripts understands the \stan's jobfile format
of storing files and flags passed to a compiler which were intercepted during
some build. It is a multi-purpose script, its default action is currently to
run \clang's static checker on files listed in a jobfile. But we used it
previously to generate preprocessed files in the Linux kernel too.

\item[\textsf{klee-log-parser}] This is a parser for the \klee output. For
each file passed to \klee, the script outputs the file along with flags. The
flags are B (an error found), C (\klee crashed), E (external call hit), H
(timeout hit), and M (memory error). If there is no flag, there was no error
found in the file.

\item[\textsf{line\_to\_fun}] It calls our \clang's static checker module
which outputs ranges of lines occupied by functions in a file. The script
converts a line number on the input to the name of a function in which the
line lays. 

\item[\textsf{llvm-builder}] After we have preprocessed files generated either
by \code{i-builder} or \code{jf-parser}, this scripts translates them all to
the \llvm bytecode (suffix \code{.llvm}).

\item[\textsf{modstats\_counter}] The \code{ModStats} module generates
statistics only for a single module. This scripts is used for comparing two
modules. Especially how many instructions were sliced and how many loops
dismissed from the sliced code.

\item[\textsf{modstats\_gen}] Before we can run \code{modstats_counter}, we
have to execute this script to generate the statistics for all modules in the
current directory and subdirectories in batch.

\item[\textsf{new\_method}] It is a script on a higher layer. It accepts on
the input a list of files, starting functions, and assertion lines which we
are currently interested in. Then it calls \code{llvm-builder}, \code{Slicer},
and \code{Kleerer}. The respective assertions are used as slicing criteria.
Hence the script is used to classify reports of other tools like \stan which
are imprecise.

\item[\textsf{stanse\_extract\_lines}] It takes the \stan's XML file with
reports (see Section~\ref{sec:stan_framework}) and generates a text file. The
text file contains a single error on a line with a trace consisting of list of
files and functions. It is useful for running \code{new_method} with this
information.
...
2.6.28, where the static analyzer \stan reported an error. More precisely,
\stan reported an error trace starting in these functions. We consulted the
reports with kernel developers to figure out which are false positives and
which are real errors. All the selected functions (and all functions
transitively called from them) contain no assembler (in some cases, it has
been replaced by equivalent C code) and no external function calls after
slicing.

We ran \symbio on these functions. All tests were performed on a machine with
an Intel E6850 dual-core processor at 3\,GHz and with 6\,GiB of memory. The
machine was running Linux. We specified parameters to \klee to time out after
...
not pass the optimize parameter to \klee, because it causes \klee to crash for
many input files. The optimization might further speed the analysis up, but
the crashes inside \klee are a run for a longer time.
...
& \textbf{Co.} & \textbf{Inst.} & \textbf{Slic.} & \textbf{SE} &
  \textbf{Total} & & & \textbf{State} \\ \hline \hline
...
\end{tabular}
\end{center}
\caption{Experimental results of \symbio. The table presents running time of
  ...
  result of \symbio: E means that the tool found a real error, FP means
...
Table~\ref{tab:results_fmics} presents results of \symbio on selected
functions.  The table shows a compilation, instrumentation, slicing, symbolic
execution, and overall running time. Further, the table presents the ratio
of instructions that were sliced away from the instrumented \llvm code.  The
last two columns specify the results of our analysis and the real state
confirmed by kernel developers. The table clearly shows that the bottleneck of
our technique is mostly the symbolic execution. Note there are exceptions in
the kernel like the function \code{qlge_set_mac_address}. There, the slicing
is more expensive due to circular inter-procedural dependencies of the slicing
criteria. Many states have to be transferred over calls until the fixed point
is reached. In any case, the time of slicing was not more than a minute.
Moreover, if we did not slice the code, the only function completely executed
in time would be \code{sisusbcon_set_palette}, computed in 20.64\,s. With the
slicing, we still see some timeouts caused by unbounded loops that had to
remain in place as they affect slicing criteria.
...
\stan. If our technique reports an error, it is a real one. If it finishes the
analysis without any error detected, the original error report is a false
positive. The analysis may also not finish in a given time, which is usually
caused by loops in the sliced code. Finally, it may report a memory error
mentioned above.

%}}}
%{{{ Related Work

\section{Related Work} \label{sec:fmics_related}

We feel the closest relation to the approach of
\textsc{Sante}~\cite{slic_sante} which emerged in almost the same as our idea.
\textsc{Sante} takes the output of a static analyzer and tries to check
whether the reported errors can be reached in real environment. The tool takes
the analyzed program, slices it and runs it dynamically. The slicing speeds up
the dynamic run and reduces the time needed for verification of genuineness.

There are many tools checking properties described by finite state machines.
The technique of \xgcc~\cite{metacomp} found a thousands of errors in real
system code.  The \textsc{Esp}~\cite{esp} technique uses a similar language as
\xgcc for properties description.  It implements an inter-procedural data-flow
algorithm based on the approach by Reps et al.~\cite{dataflow_large} for error
detection and an abstract simulation pruning algorithm for false positives
suppression.  \stan~\cite{stanse} (Chapter~\ref{sec:stanse}), a static
analysis tool also uses state machines for description of checked program
properties. The description is based on parametrised abstract syntax trees.
Finally, \textsc{Cegar}~\cite{cegar} based tools like
\textsc{Slam}~\cite{slam_decade, slam2}, \textsc{Sdv}~\cite{sdv}, or
\textsc{Blast}~\cite{blast}, do not produce false positives, in theory.
However, to achieve an appropriate efficiency and scalability for a practical
use, the implementation of the \textsc{Cegar} loop is typically unsound.

Program analysis tools based on the symbolic execution~\cite{se_king} mainly
discover low-level bugs like division by zero, illegal memory access,
assertion failure etc. These tools typically do not have problems with false
positives, but they have problems with scalability to large programs. There
were developed a lot of techniques improving the scalability to programs used
in practice. There are techniques coping with the scalability as we discussed
in Section~\ref{sec:se}. But neither of those is really related to our
technique implemented in \symbio.

%}}}
%{{{ Future Work

\section{Future Work}\label{sec:fmics_future}
Our future work has basically three independent directions.  First, we plan to
run our tool to classify all lock-related error reports produced by
\textsc{Stanse} on the Linux kernel. The results should provide a better image
of practical applicability of the technique. To get a relevant data, we should
solve some practical issues like a correct detection of starting functions,
automatic replacement of assembler, treatment of external function calls, etc.
We should also implement an on-demand memory allocation to \klee as discussed
in Section~\ref{sec:fmics_impl} or use a different executor.
...
\noindent Repository (Scripts): \url{https://github.com/jirislaby/AI_slicing} \\
