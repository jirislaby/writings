...
2.6.28, where the static analyzer \stan reported something. More precisely,
\stan reported an error trace starting in these functions. We consulted the
reports with kernel developers to sort out which are false positives and which
are real errors. All the selected functions (and all functions transitively
called from them) contain no assembler (in some cases, it has been replaced by
equivalent C code) and no external function calls after slicing.
...
not pass optimize option for \klee because it causes \klee to crash for most
of the input. The optimization might further speed the analysis up. The
crashes inside \klee are yet to be investigated.
...
\code{sisusbcon_set_palette}, computed in 20.64\,s.
