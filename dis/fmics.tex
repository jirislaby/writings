...
bytecode before it is passed to \klee. In particular, the module
...
memory at negative index). The idea behind is explained in
\ucklee~\cite{ucklee}.  Finally, the resulting bytecode is symbolically
executed by \klee. If a symbolic execution touches a memory out of the
allocated area, we get a \emph{memory error}. To remedy this inconvenience, we
plan to implement the same on-demand memory handling
\ucklee does.
