...
parsing whichever program with sources in a specified language.  Second, the
tools are not applicable to real software due to high computation time needs
(cf.~symbolic execution started being used almost 30 years after being
introduced). Further, the tools need not find any errors in large projects.
This happens when the tools, or the projects, are highly specialized for
example.  In this thesis we aim at mapping and investigation of current
techniques used for code analysis and their application on real, large
software projects.
...
privilege levels applications run in~\cite[pages 21--23]{os_concepts}.
Kernels of the operating systems often run at the level with no limitations,
i.e.~in the \emph{kernel mode}.  There, the kernel can touch any memory in a
machine and can also change other possible resources the kernel is able to
reach. User space programs on the other hand tend to be restricted, i.e.~they
run in the \emph{user mode}. There, a process cannot change any system
resource or memory of others, if not explicitly allowed to, e.g.~by shared
memory~\cite[pages 124--126]{os_concepts} or special system calls~\cite[pages
62--73]{os_concepts}.  In particular, whether some code runs in the kernel or
user mode has a direct impact on the effect caused by a potential error in the
code. A crash of the kernel means the machine cease production any new
actions, whereas a crash of a user space application causes mostly only pain
to its user.  So at the first glance it looks like we should specifically aim
at kernels of operating systems. But think about a vulnerability in a random
web browser or database system (which consist of millions lines of code too).
It could indeed lead to stolen user's personal data or even money from their
bank accounts. The same as a problem in the kernel.
...
exploit_proto_ops, exploit_userns}\cite[page 256]{hacking}, and more to be
