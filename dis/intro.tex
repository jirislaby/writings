...
introduced \textsc{Sdv} toolkit~\cite{sdv} based on
\textsc{Slam}~\cite{slam_cav, slam_popl, slam_decade} and later \textsc{Sdv}
2.0 with \textsc{Slam2}~\cite{slam2} to improve situation around Windows
...
demonstrated in \sage~\cite{sage} for example. Second, we can ignore calls to
unknown code. But we lose precision of course. Finally, we can solely aim at
projects that are self-contained. They do not depend on any other code.  These
are kernels of operating systems for example.
