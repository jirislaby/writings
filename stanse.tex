...
structures are shared by all running checkers. Moreover, \Stanse can run in a
command line mode or via a fully featured GUI. If properly configured, \Stanse
can be used as a ``push-the-button'' tool. Finally, the \Stanse framework has
been designed as a Java library, so it can be easily transformed into a static
analysis plug-in for Eclipse or NetBeans IDE.
...
we present \Stanse results when applied to version 2.6.28. We focused on checking of the
following properties:
\begin{itemize}
\item Correct pairing of functions (balanced locking, reference counting)
\item Pointer manipulation (dereferencing null or dangling pointers, etc.)
\item Atomic section restrictions (deadlocks caused by sleeping inside spinlocks or interrupt handlers)
\item Thread safety properties (deadlocks )\todo{What's the difference between point 3 and 4?}
...
The first three properties are checked by \AC, the fourth and fifth property is checked by \TC and \RC respectively. 
...
was between 400 and 1000\,MiB. The false positives rate differs for different properties, but the overall ratio is about 50\,\% of all sorted\todo{What to choose instead of sorted?} errors reports.  Bugs were reported to kernel developers and fixed in the following kernel
releases. Some of these bugs remained undiscovered for seven
years\footnote{\url{http://lkml.org/lkml/2009/3/11/380}}.
Table~\ref{tab:results} shows frequencies of error reports for all checked properties.
...
\textbf{Checker} & \textbf{Property} & \textbf{Reports} & \textbf{Errors} & \textbf{FP} & \textbf{\% FP} \\ \hline\hline
\textit{Automaton} & \textit{Pairing}
                             & 266 & 65  & 143 & 68.8\,\% \\ \hline
& \textit{Memory}            & 86  & 48  & 37  & 43.5\,\% \\ \hline
& \textit{Atomic}            & 35  & 16  & 18  & 52.9\,\% \\ \hline
\textit{Thread}        & n/a & 20  & 9   & 11  & 55.0\,\% \\ \hline
\textit{Reachability}  & n/a & 751 & 751 & 0   & 0.0\,\%  \\ \hline\hline
\textbf{Overall}       &     & 407 & 138 & 209 & 51.4\,\% \\ \hline
\end{tabular}
\end{center}
\caption{\Stanse results on Linux kernel version 2.6.28. Not all
of the error reports (column {\bf Reports}) were sorted to real errors (column
{\bf Errors}) and false positives (column {\bf FP}). Column {\bf \% FP}
represents percentage of false positives in all sorted reports.
\textbf{Overall} does not involve \textit{Reachability} results (it would
distort the counts).}
\label{tab:results}
\end{table}
\todo{Reachability is not included in Table \ref{tab:results}. It looks bad.}
...
bug-finding algorithms based on static analysis. The framework focuses on
