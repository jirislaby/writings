...
hidden to checkers.\todo{Probral jsem to se zenou a oba si myslime, ze
  ...
  neni pravda.}\note{No jo, ale s hidden to vypada, ze to zase nemuzou pouzit
  (je to pro ne schovany). Mozna spis invisible, nebo nejaky pekny vyraz pro
  ,,odstinit''?}
...
checkers to be used\note{Tomuto nerozumim, jake configuration files? Jedine,
co to umi je to vyspecifikovat soubory k analyze na cmdline. Konfigurace
checkeru musi byt v souborech vzdy\ldots}.
Further, the framework supports parallel execution of
...
An important reason why we focus on analysing the Linux kernel is that it
represents a large and freely available codebase, where the absence of bugs is
of great concern.  Moreover, the Linux kernel fully exercises most features of
ANSI C99 and GNU C extensions. The other motivation is to provide a free tool
for Linux kernel developers.

We have been applying \Stanse on the Linux kernel since version 2.6.28.
Here we present our results of applying \AC exactly on the 2.6.28. We checked
...
4\,GiB of memory was under 100 minutes. The memory usage of the Java process
oscillates between 400 and 1000\,MiB in case of forced garbage collector
executions. The false positives rate oscillates among different bug types, the
overall ratio is that 40\,\% of all reported errors are real bugs.  Bugs were
reported to kernel developers and fixed in the next kernel release. Some of
these bugs remained undiscovered for seven
years\footnote{\url{http://lkml.org/lkml/2009/3/11/380}}.
Table~\ref{tab:results}\note{Popisek u tabulky je spatne -- naopak. Chceme v
tabulce radsi procento FP (v popisku), nebo chyb (v tabulce)} shows results
from \AC for all three checked properties.
...
\end{tabular}
\end{center}
\caption{Errors found by \AC in the 2.6.28 Linux kernel. Not all of the error
reports (column {\bf Reports}) were sorted to real error (column {\bf Bugs})
or false positives (column {\bf FP}). Column {\bf \%} represent percentage of
false positives in all sorted reports.}
\label{tab:results}
\end{table}
...
Even if \Stanse with its three simple bug-finding algorithms has already found
more than 130 real bugs in the Linux kernel and this number is growing every
month, there are still many aspects that should be improved. For example, we
are currently working on C++ support (note that the framework can handle any
programming language for which a frontend is written). Further, we plan to
provide \Stanse in the form of a plug-in for Eclipse or NetBeans IDE. A lot of
work can be done in the area of automatic false-alarm filtering and error
...
written in Java by an optimized parser written in C, to replace the XML format
of internal structures by something more concise, to add a support for
function summaries, etc.
