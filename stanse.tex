...
\item \Stanse is a robust framework (written predominantly in Java) for
  ...
  \item As \Stanse already contains three checkers, it can be also seen as a
...
Architecture of the framework is depicted in Figure~\ref{fig:architecture}. We
describe its individual parts in separate subsections.
...
\Stanse framework contains data structures capturing a configuration of an analysis to be performed. We always need to know what source files to analyze and by what checkers. That information we call the \emph{configuration}. 
...
We also need to specify checkers which should be applied to chosen source
files. We can see the list of selected checkers on the top of
Fig.~\ref{fig:architecture}. There can be many checkers running
simultaneously in \Stanse. But they do not see each other. They proceed
independently. Checkers themselves might require their own configuration
files. They are depicted at the top right corner of
Fig.~\ref{fig:architecture}.

The configuration can be passed to the \Stanse framework via command line
arguments. User can also add them in the graphical interface.
...
The code is then parsed into the \emph{internal structures}: a
call-graph, a CFG for each function, and an AST for the whole file.
Subtrees in the AST are referenced from appropriate CFG nodes.
% These structures are linked in a natural way.
We show these structures in the middle of Fig.~\ref{fig:architecture}.
