...
including the list of source files to be analyzed and checkers to be
...
We have been applying \Stanse on the Linux kernel since version 2.6.28. Here
we present our results of evaluation \Stanse's checker \AC just on version
2.6.28\note{Tahle veta mi nedava moc smysl}. We focused on checking of the
...
executions. The false positives rate oscillates among\todo{oscillates
  according je blbe, i ten zbytek je formulovanej divne.} the different bug
types, the overall ratio is that 40\,\% of all reported errors are real
bugs.  Bugs were reported to kernel developers and fixed in the next kernel
release. Some of these bugs remained undiscovered for seven
years\footnote{\url{http://lkml.org/lkml/2009/3/11/380}}.
Table~\ref{tab:results}\todo{v popisku tabulky nahradit sorted} shows results from \AC for all three checked
properties.
...
\end{tabular}
\end{center}
\caption{\AC evaluation results on version 2.6.28 of the Linux kernel. Not all
of the error reports (column {\bf Reports}) were sorted to real errors (column
{\bf Bugs}) and false positives (column {\bf FP}). Column {\bf \% FP}
represents percentage of false positives in all sorted reports.}
\label{tab:results}
\end{table}
...
month, there are still many aspects that should be improved.\todo{tu prvni
vetu jeste predelat - chvalu do prvniho odstavce, tady nechat jen future
work.} For example, we are currently working on C++ support (note that the
framework can handle any programming language for which a frontend is
written).\todo{Lepe prodat?} Further, we plan to provide \Stanse in the form
of a plug-in for Eclipse or NetBeans IDE. A lot of work can be done in the
area of automatic false-alarm filtering and error importance classification.
