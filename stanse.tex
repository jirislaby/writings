...
\Stanse framework is modular and fully open. It is designed to allow static analysis of large software projects (like Linux kernel), and to reduce an effort when implementing a new static analysis technique.
% An integration of a new checker into the framework is very easy and the checker immediately gains whole functionality of the framework discussed bellow.
Architecture of the framework is depicted in Figure~\ref{fig:architecture}. We describe its individual parts in separate subsections.

 %matching formal and actual function parameters
...
functionality is inspired by the \textsc{Sparse}~\cite{sparse} tool. At bottom left corner of Figure~\ref{fig:architecture} there we show a list of \texttt{.C} files which are identified in the configuration.

We also need to specify checkers which should be applied to chosen source files. We can see the list of selected checkers in the top of Figure~\ref{fig:architecture}. There can be many checkers running simultaneously in \Stanse. But they do not see each other. They proceed independently. Checkers themselves might require their own configuration files. They are depicted at top right corner of Figure~\ref{fig:architecture}.

The configuration can be passed to \Stanse framework via command line arguments or edited in GUI application.
...
ISO/ANSI C99 standard and most of the GNU C extensions. This allows \Stanse to process software projects like Linux kernel. We are currently working on support of other languages, in particular C++. We have a prototype implementation, but it is not included in the distribution yet. The parsing pipeline, including a preprocessing the source files by the standard GNU C preprocessor, is depicted bottom in Figure~\ref{fig:architecture}. It is important to note that we do not parse all the sources in a sequence one by one as they appear in a configuration. Since we use streaming (discussed later), the pipeline is applied several times independently for individual source files.
...
We show these structures in the middle of Figure~\ref{fig:architecture}.
All these structures can be dumped in a textual or graphical form.

Since we aim at large software projects consisting of hundreds or
thousands of modules (compilation units), it is often impossible in practice to store the corresponding internal structures in the
memory all at once. The \Stanse framework therefore applies automatic \emph{streaming} of the internal structures, currently on a module basis.
...
%analysing large software projects.
%
...
\subsection{Traversing Internal Representation}\label{ssec:travers}

Although a checker may need to work with the internal structures in an arbitrary way, most of checkers walk through CFGs using some standard strategy. To prevent
...
\end{itemize}
This makes implementation of new algorithms extremely simple. The discussed functionality is shown to the left of internal representation in Figure~\ref{fig:architecture}.
...
In \Stanse framework a checker is an implementation of some concrete static analysis technique. Each checker has an access to shared internal structure of analysed source files. They are also gained an access to the algorithms providing navigation in those structures. This is done by an interface between checkers and internal structure and libraries of the framework. The interface is depicted in Figure~\ref{fig:architecture} right bellow the checkers.

The checkers are integrated to the framework of \Stanse using concrete factory design pattern. Therefore, to insert a new checker to the framework of \Stanse one needs to implement general checker interface, and register it to the checkers' factory of the framework. Then it gains a full access to the features of the framework accessible through the discussed interface.
...
Error reporting and tracing pipeline is depicted to the right of interface and internal representation of Figure~\ref{fig:architecture}.
...
\multicolumn{2}{|l|}{\textbf{Overall}}  & \textbf{438} & \textbf{169} &
\textbf{209} & \textbf{44.7\,\%} \\ \hline
\end{tabular}
\end{center}
...
company, and a handful of Linux kernel developers. Several other kernel
developers, in whose code \Stanse has found bugs, have also expressed interest
in using \Stanse regularly\footnote{\url{http://lkml.org/lkml/2009/7/18/59}
and follow-ups}. We therefore expect the number of users to go significantly
up with the first full release 1.0.0.
