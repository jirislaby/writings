...
Once a checker finds an error, it reports it back to the framework as a commented \emph{error
  trace} (a path in the analysed code) demonstrating this error. \Stanse framework provides a data type for describing an error. 
...
In the framework there are then several possibilities how to present the error traces back to the user of \Stanse: they can
be printed to the console, displayed using a built-in error trace browser in
the GUI (see Figure~\ref{fig:bugtrace}), or saved to an external file in XML
...
database via a web browser. Using the web browser or the built-in graphical
error browser, one can mark errors as real bugs or false positives. \Stanse
also provides various statistics of errors like number of errors per checker,
...
In this section we briefly describe the three currently available checkers.
...
information, it count in how many cases each variable is accessed while some
lock is held. If some variable is changed 99 times while some lock is held and
in one it is not, it is reported as possible error. The boundary is currently
set to 70, so that at least 70\% of accesses must be under locks. The rest
(30\%) is then reported. This work is based upon~\cite{lockchecker}.
...
development (there is a constant income of new bugs), and absence of bugs is of great
concern. 

We applied \Stanse with the three checkers described in the previous section
to version 2.6.28. \AC uses three automata describing the following
kinds of errors:
\begin{itemize}
...
overall ratio of real errors to all classified errors is not high: 44.7\%.
