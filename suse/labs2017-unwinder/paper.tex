...
and put all function addresses to a call trace. Function address is easy to
identify in the kernel. It is an address either inside kernel or inside
modules. And given the stack contains return addresses, they will be surely
part of the call trace. And of course with other random material found on the
stack which by a chance fits in the kernel code space.

This approach is used whenever any of the sophisticated methods below are
available. Since this is the most simple and naive, it is the fastest one. But
least precise, of course.
...
be used to identify the start of stack frames and locate return address
precisely.
...
up to 10\,\%~\cite{fp_slowdown} -- depending on the load.

\section{Rewrite of Unwinding} \label{sec:rewrite}
As an ongoing effort to use unwinding from Kernel Live Patching, the unwinding
in the upstream kernel was completely redesigned in 4.9 by commit
\commit{7c7900f89770} (x86/unwind: Add new unwind interface and
implementations)~\cite{git_unw_rewrite}.
