...
Section~\ref{sec:pre_suse} for more information.
...
There is a run-time penalty stemming from enabled frame pointers. First,
functions have to contain prologues and epilogues to maintain the frame
pointer (\code{BP}). Second. The slowdown is approximately
10\,\%~\cite{fp_slowdown}

\subsection{Pre-DWARF Unwinding} \label{sec:predwarf}
DWARF in SLE11 and SL-11.1 since d7af6f6e8a34.

\subsection{openSUSE \& SLE} \label{sec:pre_suse}

Due to 131484c8da97 in 4.2, SLE12-SP2 \& FP enabled.

\section{Post-rewrite Unwinding} \label{sec:rewrite}
