...
in the competition benchmarks. All incorrect answers produced by the tool are
due to bugs in \textsc{Clang} 3.1 (broken \texttt{inline} handling), \textsc{Klee}
...
We decided to opt-in to 4 categories:
\emph{ControlFlowInteger-\-MemPrecise}, \emph{DeviceDrivers64},
\emph{FeatureChecks}, and \emph{SystemC}. \tool was able to finish the
computation and provide a result \texttt{SAFE} or \texttt{UNSAFE} for 666
out of 1465 files. Out of these 666 answers, 40 are incorrect.
%  In the category with best results
% (\emph{FeatureChecks}), \tool answered 68\% correctly.
% The remaining benchmarks in the four cat still time out during the symbolic
% execution or cause a crash in \textsc{Klee}.
