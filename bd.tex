...
\begin{abstract}
  ...
  of various program analysis tools. The database currently contains a heap
  of bug reports produced by several tools applied to the Linux kernel. The
  reports are classified as either real errors or false positives. The
  database can be downloaded and manipulated by SQL queries, or acesses via
  a web frontend. Using the database, developers of various bug-finding
  tools can easily evaluate performance of their tool on a large real-life
  software project.
\end{abstract}
...
part of real errors. To improve or evaluate such a tool, one needs a
suitable benchmark, i.e.~a set of programs with a (preferably complete) list
of their errors. There exist benchmark suites consisting of small synthetic
programs~\cite{CHK09,KRA05,CHK11,NCH05,SRD} and those consisting of
real-world programs~\cite{CHK09,LLQ05,HW08}. In benchmark
suites~\cite{KRA05,CHK11,NCH05}, relevant program locations in small
synthetic programs are explicitly marked as either erroneous or safe. The
benchmark suite~\cite{CHK09} marks only real errors. The situation is
different for benchmarks with real-world programs: \cite{LLQ05} contains
marked test cases triggering errors and non-triggering errors,
while~\cite{HW08} provides bug-reports (both real errors and false
positives) collected from \textsc{Findbugs} and sorted according to priority
levels assigned to the reports by the tool. The benchmark suites discussed
so far considers different error types and provide their own error
taxonomies with exception of suites \cite{CHK09,CHK11}, where errors types
are linked to \textsc{CWE}~\cite{cwe}.

As far as we know, there is no big real-life project with a significantly
long list of uniformly described bug reports classifed as real errors or
false positive. Our benchmark suite \cdb fills this gap. The database
currently contains only a single project, namely the Linux kernel 2.6.28.
We have collected over 600 bug reports of 11 error types produced by several
bug-detection tools. The reports have been manually classified by skilled
programmers with a help of Linux kernel developers as real errors or false
positives. % Only a few of them remain unclassified.

To evaluate a bug-finding tool, one can analyse a project from our benchmark
and compare the produced bug reports with these in our database. In this
way, one immediately gets classification of the reports matching database
entries and also information about missed real errors. This information can
be further processed according to methodologies of~\cite{HW08,CHK09}.

The database can be downloaded in \textsc{SQLite~3} format for local use
...
projects and their version. We decided to have each project and version in a
separate database. Hence the user selects project and version and then works
with a chosen project/version. Such sub-database comprises information about
errors, tools, error types, users, and relation between them. The main tables
in the database are two:
\begin{itemize}
\item \textbf{error\_type} -- specifies various kinds of errors (cf.
Section~\ref{sec:error_type}). It contains a name of the type, short
description and a reference CWE number if exists (see later).
\item \textbf{error} -- an error specified by the type above, location (file
and line), URL for reference (to provide more info), classification (false
positive, real error, unclassified), confirmation (how the bug was fixed, a
commit ID for instance), user who inserted the entry, and timestamp of the
moment of insertion.
\end{itemize}
...
All the tools we have an output from, produce only a limited count of error
types. We investigated their output and established a set of error types to
put into the database. Since there already exists a publicly available database
of error types, we decided to leverage that. It summarizes the error type,
provides examples and also points to real errors of a particular type. The
database is called \emph{Common Weakness Enumeration} (CWE)~\cite{cwe}.
However, two of our types are not present in CWE. We consulted their inclusion
in CWE, but it was concluded that the are too project-specific and will not be
added. Here, we list all error types currently available in \cdb. The line of
each of the reports points to the original source file, even though a macro
expansion may happen at that place.
...
line where the assertion appears in the code.

\noindent \textbf{Double Lock} (CWE 764) \\ \hangindent=2em
Some lock is unintentionally locked by a thread twice in a sequence and it
leads to an inconsistent lock state. The location is specified by the line
where the second lock happens.

\noindent \textbf{Double Unlock} (CWE 765) \\ \hangindent=2em
Some lock is unintentionally unlocked by a thread twice in a sequence and it
leads to an inconsistent lock state. The location is specified by the line
where the second unlock happens.

\noindent \textbf{Use After Free} (CWE 416) \\ \hangindent=2em
Dereferencing a pointer that points to an already freed memory. The location
is specified by the line of the use.
...
the line of the invalid (second) free.
...
The code requests one copy of a resource from a system such as structures
holding hardware status, but there is a try to return that to the system
twice. The location is specified by the line of the second put.
...
may become unresponsive or crash. The location is specified by the line of the
function call.
...
Sometimes, this is reported by tools which analyze a project per module and
start the analysis from some chosen function. If the lock is locked while
leaving from the function, this report is generated.  The location is
specified by the line of \texttt{return} statement or closing brace if there
is no \texttt{return}.

The last two types may be only Linux kernel specific. As can be seen the
database is not limited to a bounded set of error types. Each project may
extend this set by its own set.

%}}}
%{{{ Current Data and Their Sources

\section{Current Data and Their Sources}\label{sec:data}
Currently, the database comprises 616 reports of 11 types.  118 of the reports
are real, 388 false positives. The database contains also 110 entries which
were left unclassified. Many of those were added even recently and are about
to be classified soon. Table~\ref{tab:results} depicts how each kind of bugs
