...
\begin{abstract}
  ...
  database can be downloaded and manipulated by SQL queries, or accessed via
...
so far consider different error types and provide their own error
taxonomies with exception of suites \cite{CHK11,CHK09}, where errors types
are linked to \textsc{CWE}~\cite{cwe}.

As far as we know, there is no big real-life project with a significant long
list of uniformly described bug reports classifed as real errors or false
positive. Our benchmark suite \cdb fills this gap. The database currently
contains only a single project, namely the Linux kernel 2.6.28.  We have
collected over 600 bug reports of 11 error types produced by several
bug-detection tools. The reports have been manually classified by skilled
programmers with a help of Linux kernel developers. Either as real errors or
false positives. % Only a few of them remain unclassified.

To evaluate a bug-finding tool, one can analyse a project from our benchmark
and compare the produced bug reports to those in our database. This way, one
immediately gets a classification of the reports matched to our database
entries and also information about missed real errors. This information can be
further processed according to methodologies of~\cite{HW08,CHK09}.

The database may be downloaded in \textsc{SQLite~3} format for a local use
...
Currently, the database comprises 616 reports of 11 types from the Linux
kernel.  118 of the reports are real, 388 false positives. The database
contains also 110 entries which were left unclassified. Many of those were
added even recently and are about to be classified soon. The following table
depicts how each kind of bugs is represented in the database.
...
\textbf{Error Type} & \textbf{Real Err.} & \textbf{False Pos.} &
	\textbf{Unclassified} & \textbf{Total} \\ \hline \hline
...
Use After Free & 0 & 3 & 0 & 3 \\ \hline \hline
