...
\begin{abstract}
  ...
  tools can easily evaluate effectiveness and performance of their tool on a
  large real-life software project.
\end{abstract}
...
marked test cases triggering/non-triggering errors,
while~\cite{HW08} provides bug-reports (both real errors and false
positives) produced by \textsc{Findbugs} and sorted according to priority
...
classified as real errors or false positive. Our benchmark suite \cdb fills
this gap. The database currently contains only a single project, namely the
Linux kernel 2.6.28.  We have collected over 600 bug-reports of 11 error types
produced by several bug-detection tools. The reports have been manually
classified as either real errors or false positives by skilled programmers with a help of Linux kernel developers.
...
methodologies of~\cite{HW08,CHK09}.
...
separate sub-database. A sub-database comprises information about errors,
tools, error types, users, and relation between them. The main tables in the
database are two:
\begin{description}
\item \textbf{error\_type} This table specifies various kinds of errors (cf.
...
\item \textbf{error} A bug report is specified by the type above, location
(file and line), URL for reference (to provide more info), classification
(false positive, real error, unclassified), confirmation (an argument
supporting the validity of the bug, e.g. a commit ID of the corresponding bug-fix), user who
inserted the entry, and timestamp of the moment of insertion.
\end{description}
...
We investigated output of available tools and established a set of error types
to put into the database. Since there already exists a publicly available
database CWE~\cite{cwe} of error types, we decided to leverage that.  However,
two of our types are not present in CWE. We consulted their inclusion in CWE,
but it was concluded that they are too project-specific and will not be added.
...
to the code is as they expected.  Sometimes these assertions are hit and
crashes are enforced, thus we include them in \cdb. The error location is
specified by the line where the assertion appears in the code.

\noindent \textbf{Double Lock} (CWE 764) \hangindent=2em
Some lock is locked by a thread twice in a sequence and it leads to an
inconsistent lock state. The error location is specified by the line with the
second locking function call.

\noindent \textbf{Double Unlock} (CWE 765) \hangindent=2em
Some lock is unlocked by a thread twice in a sequence and it leads to an
inconsistent lock state. The error location is specified by the line with the
second unlocking function call.

\noindent \textbf{Use After Free} (CWE 416) \hangindent=2em
Dereferencing a pointer that points to an already freed memory. The error
location is specified by the line of the use.

\noindent \textbf{Double Free} (CWE 415) \hangindent=2em
Freeing function is called twice on the same address. Most allocators can
detect this and usually kill the program. The error location is specified by
the line of the invalid (second) free.

\noindent \textbf{Memory Leak} (CWE 401) \hangindent=2em
Some code omits to free a memory which was allocated. The error location is
specified by the line of the alerror location.
...
NULL. The error location is specified by the line of the dereference.
...
status) from a system, but there is a try to return that to the system twice.
The error location is specified by the line of the second put.
...
the system for reuse. The error location is specified by the line of the
request.
...
may become unresponsive or crash. The error location is specified by the line
...
leaving from the function, this report is generated.  The error location is
specified by the line of \texttt{return} statement or closing brace if there
is no \texttt{return}.

The last two error types may be only Linux kernel specific. Note that the
...
The database comprises 616 reports: 118 real errors, 388 false positives, and
110 entries which are unclassified. Many of the unclassified entries were
...
Double Unlock & 11 & 38 & 82 & 131 \\ 
Leaving function in locked state & 27 & 150 & 0 & 177 \\ 
