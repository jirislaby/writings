...
part of real errors. To improve or evaluate such a tool, one need a suitable
...
small programs and/or artificially generated programs. \textbf{(Sem dat
...
enlarge and open our database of Linux kernel (version 2.6.28) errors to the
public. The database contains bug reports produced by several bug-detection
tools. The reports have been manually classified by skilled programmers and
a Linux kernel developer as real errors or false positives (only a few of
them are remain unclassified). Presence of false positives in the database
partly solves the problem that our database probably do not contain all
...
\textsc{SQLite~3} format for local use under the \emph{Open Database License
  (ODbL) v1.0}~\cite{ODbL}. The database (including a documentation) is
available at:
\begin{center}
...
projects and their version. We decided to cut the database at the project and
its version level. Hence the user selects project and version and then work
with a chosen sub-database. Sub-database comprises information about errors,
tools, error types, users, and relation between them.
