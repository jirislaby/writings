...
types. We investigated their output and established a set of errors to put in
the database. Since there already exists a publicly available database of
error types, we decided to leverage that. It summarizes the error type,
provides examples and also points to real errors of a particular type. The
database is called \emph{Common Weakness Enumeration} (CWE)~\cite{cwe}. Some
of our errors are not present in CWE though. For those we obviously do not
provide a CWE number. Here, we list all error types currently available in
\cdb. The line of each of the reports points to the original source file,
even though a macro expansion may happen at that place.
