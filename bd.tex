...
\subtitle{Tool for program analysis tool developers}
...
\begin{abstract}
  ...
  of various program analysis tools. The database contains bug-reports
  ...
  currently contains more than 800 bug-reports detected in Linux kernel
  ...
  SQL queries, or accessed via a web frontend. Using the database,
  developers of various bug-finding tools can easily evaluate effectiveness
  and performance of their tool.
  % The database currently contains a heap
...
without any differentiation) produced by \textsc{Findbugs} and sorted
...
where errors types are linked to \textsc{CWE}~\cite{cwe}.
...
Linux kernel 2.6.28. We have collected over 850 bug-reports of 11 error
...
The database can be accessed via a web interface at
\begin{center}
  \url{http://claburedb.fi.muni.cz/}
\end{center}
or downloaded in \textsc{SQLite~3} format for a local use
under the \emph{Open Database License v1.0}~\cite{ODbL}.
...
to improve the web interface to allow other users to add and maintain contain
of the database.
...
database including considered kinds of errors, overview of collected
...
projects and their version. As projects can be written in various
programming languages, they can contain very specific kinds of errors, and
they can be maintained by and interesting for diverse user groups, we
decided to have each project and version in a separate sub-database.
\todo{Opravdu chceme mit i kazdou verzi ve vlastni databazi? A chceme tomu
  rikat sub-database nebo database?}  A sub-database comprises information
about error types, errors, tools, users, and relation between them. The main
tables in the database are two:
\begin{description}
\item \textbf{error\_type} This table keeps a specification of various kinds
of errors. We will outline some of possible types later in
...
\item \textbf{error} A bug report is the basic unit in this table. It is
specified by the type above, location (usually file and line), URL for
reference (to provide more info), classification (false positive, real error,
unclassified), confirmation (an argument supporting the validity of the bug,
e.g. a commit ID of the corresponding bug-fix), user who inserted the entry,
and timestamp of the moment of insertion.
\end{description}

Except the data are in the database and can be accessed in raw using SQL, they
can be also viewed using the web interface as is presented in
Fig.~\ref{fig:error}.\todo{Chce to obrazek s vice chybama, kde bude i
  nejakej false positive a nebude rozbalenej prvni ani posledni zaznam}.  
...
error occurrence in Table~\ref{tab:stats}. Based on the findings of the tools
we used, we can see that the highest ratio of errors per file is in the
architectural code (x86 in our case).
