...
\begin{abstract}
  ...
  developers of any bug-finding tool can easily evaluate effectiveness
...
\end{center}
or downloaded in an \textsc{SQLite~3} format for a local use under the
\emph{Open Database License v1.0}~\cite{ODbL}.

The database is still developing in several directions: we plan to add more
bug-reports for the Linux kernel, to support another software projects, and
to improve the web interface to allow other users to add and maintain contents
of the database.
...
bug-reports and their sources. Section~\ref{sec:use} suggests a possible use
...
maintained by different people and are interesting for distinct user groups,
we decided to have each project and version in a separate sub-database.
\todo{Opravdu chceme mit i kazdou verzi ve vlastni databazi? A chceme tomu
  rikat sub-database nebo database? -- Tak jsem se domluvili. Ten hlavni duvod
  byl, ze vezmes jednu verzi SW, tu projedes a pak vezmes odpovidajici
  databazi a projedes to oproti ni. Subdatabaze jsem tady zavedli kvuli lepsi
  pochopitelnosti. Bylo to dost divoke, protoze jsme pred tim pretezovali
  vyraz databaze.} A sub-database comprises information
...
Fig.~\ref{fig:error}. Here we see a list of seven Double Resource Put reports.
Four of them are false positives (green), the other three are real errors
(orange). One of the reports is expanded, so that we can see its details and
