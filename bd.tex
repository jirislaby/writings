...
i.e.~classify them as false positives or real errors, and find errors in the
sources that were missed by the tool. This work is usually tedious and time
consuming, especially when one tunes or studies performance of a tool for real
software projects. The tedious work can be avoided if suitable benchmarks,
...
\section{Current Content of the Database}\label{sec:data}
...
2.6.28. We have chosen Linux kernel for several reasons: it is a big (7310
files with nearly 7 milions LOC in total), real-life, self-contained, and
...
drivers, filesystems, networking etc.) that covers some standard application
...
\clang~3~\cite{clang}, \stanse 1.2~\cite{stanse}, and one commercial tool
...
type. This is to evade a misinterpretation as diverse tools can associate
...
\item \textbf{BUG/WARNING} (CWE 617) Developers often inject \emph{assert}s
  to their code, e.g.~to ensure the their function gets a correct input.
  ...
  destroyed is inactive. An error occurs if the condition of some
...
\item \textbf{Memory Leak} (CWE 401)
  ...
  \emph{Error location:} the line with the allocation command.
\item \textbf{Invalid Pointer Dereference} (CWE 465) The code tries to
  ...
  in many ways. For example, the pointer may point to a deallocated memory
  (known as \emph{dangling pointer} or \emph{use after free}) or it can be
  set to NULL (known as \emph{NULL pointer dereference}) or
  uninitialized. Another source of the problem may be
...
\item \textbf{Calling function from invalid context} (not in CWE, Linux
  kernel specific) Some function is called at inappropriate place or within
  ...
  considered to be an error because results of such a call are
  undefined: the system may become unresponsive or may crash for instance. \\
...
\item \textbf{Leaving function in locked state} (not in CWE, Linux kernel
  ...
  an error (violation of kernel coding policy) as such an execution may lead
  to a deadlock on the next invocation of any function wanting to take the
  same lock. \\
...
database. As seen from the table, most of the bug-reports in the database
...
\section{Intended Use of the Database}\label{sec:use}
...
from our benchmark suite and run the tool on these source codes. Let us note
that \cdb contains only open-source projects. As the second step, the
sub-database of classified bug-reports corresponding to the chosen project
is downloaded from \cdb. Note that installation and local database usage is
described in \cdb documentation.

The bug-reports produced by the tool are compared to the downloaded
data. This way, one immediately gets a classification (real error/false
positive) of all the reports matched in the database and also information
about some missed real errors. It remains to manually classify the
bug-reports not matched by the database. The numbers of produced false
positives, detected and missed real errors can be further statistically
processed according to methodologies of~\cite{CHK09,HW08} which produce
several metrics including accuracy and precission. The missed errors and
false positives are valuable inputs for tuning the tool. 

Finally, user is kindly asked to insert newly discovered bug-reports into
the database.  
...
real-life project, namely Linux Kernel 2.6.28.
...
% one program point. 
Further, we plan to add other real-life projects. We also plan to
extend the web interface to enable other users to maintain and extend the
database.

In general, the future of \cdb depends on feedback of the program analysis
community. At this point, we would like to encourage you to contact us at
\texttt{claburedb@fi.muni.cz} if you have any comments, suggestions,
questions, or sources of bug-reports for Linux kernel or other interesting
projects. We would especially welcome people willing to participate on 
creating and maintaining the database content for another project.
