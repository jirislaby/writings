...
Here, we list all error types currently available in \cdb. Along with all the
types we also include a description of the actual error location. It may be
sometimes unclear what code line is connected to a particular error type.
...
crashes are enforced, thus we include them in \cdb. A typical use case might be
when we implement a destroy function of an object and we want to ensure the
object is not alive by a check like \texttt{BUG\_ON(obj->active)}. \\
The error location is the line where the assertion appears in the code.

\noindent \textbf{Division by Zero} (CWE 369) \hangindent=2em
The code contains a division instruction but the actual value of the divisor
is zero. \\
The error location the line where the division appears in the code.
...
lock state. \\
The error location is the line with the second locking function call.
...
\noindent \textbf{Invalid Pointer Dereference} (CWE 465) \hangindent=2em
...
(it is NULL or undefined value). Also a source of the problem might be
...
status) from a system, but there is more than one attempt to return the
resource back to the system. Like in this example:
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}[language=C]
struct pci_dev *pdev = pci_get_device(vendor, device, NULL);
if (pdev) {
  work_with_pdev(pdev)
...
\hangindent=2em The error location is the line of the second put.
...
The error location is the line of the request. \\[0.5ex]
...
\noindent \textbf{Calling function from invalid context} (not in CWE)
\hangindent=2em
Some function is called at inappropriate place. This includes calling
functions like sleep or wait inside spin-lock critical sections or in
interrupt handlers. If that happens, results are undefined, the system may
become unresponsive or may crash for instance. \\
The error location is the line of the inappropriate call.

\noindent \textbf{Leaving function in locked state} (not in CWE) \hangindent=2em
There is a path in a function where some lock is locked but unintentionally
left locked when returning. This often results in deadlocks on next
invocation of any function wanting to take the same lock. This error type
stems from the kernel requirements when returning to userspace that a process
cannot hold any locks. And yet, this is sometimes reported by tools which
analyze a project per module and start the analysis from some chosen starting
functions.  If the lock is locked while leaving these functions, this report
is generated.  \\
The error location is the line of \texttt{return} statement or closing brace
if there is no \texttt{return}.

\subsection{Errors} \todo{update}
...
\end{center}
\caption{Error list with one error expanded}
\end{figure}

\todo{stats (file count, size, avg, med, err per file, err per file type)}
